...
class Application(tk.Tk):
    def __init__(self, matrix, rows, columns):
        ...
        self.canvas = tk.Canvas(self, width=635, height=570 + 58, bg="#a4d5ff", highlightthickness=0, borderwidth=0,
                                highlightbackground="#127bc7", relief='flat')
        self.canvas.pack()
        ...
        for row in range(11):
            for col in range(11):
                ...
                if row == 10 and col == 10:
                    ...
                    text_y = (y1 + y2) // 2
                    self.canvas.create_rectangle(x1, y1, x2, y2, outline="#127bc7", width="0")
                    self.canvas.create_text(text_x, text_y, text=self.columns[col], font=("Arial", 22),
                                            fill=get_color(self.columns[col]))
                elif col == 10:
                    text_x = (x1 + x2) // 2
                    text_y = (y1 + y2) // 2
                    self.canvas.create_rectangle(x1, y1, x2, y2, outline="#127bc7", width="0")
                    self.canvas.create_text(text_x, text_y, text=self.rows[row], font=("Arial", 22),
                                            fill=get_color(self.rows[row]))
                else:
                    self.canvas.create_rectangle(x1, y1, x2, y2, outline="#127bc7", width="3")
                    image_x = (x1 + x2) // 2
                    image_y = (y1 + y2) // 2
                    image = ''
                    if self.matrix[row][col].upper() == 'W':
                        image = self.agua
                    elif self.matrix[row][col].upper() == 'T':
                        image = self.cima
                    elif self.matrix[row][col].upper() == 'B':
                        image = self.baixo
                    elif self.matrix[row][col].upper() == 'R':
                        image = self.direita
                    elif self.matrix[row][col].upper() == 'E':
                        image = self.esquerda
                    elif self.matrix[row][col].upper() == 'C':
                        image = self.circulo
                    elif self.matrix[row][col].upper() == 'M':
                        image = self.centro
                    elif self.matrix[row][col].upper() == 'L':
                        image = self.esquerda
